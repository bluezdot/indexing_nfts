...
// CREATE TABLE //
createTableQuery = `CREATE TABLE IF NOT EXISTS Aspo_aspo (
...
async function main() {
    ...
    const nftAbi = process.env.ASPO_ABI_NFT;
    const nftContract = new ethers.Contract(nftAddress, nftAbi, provider);

    const tokenAddress = process.env.ASPO_ERC20_CONTRACT;
    const tokenAbi = process.env.ASPO_ABI_ERC;
    ...
    myLogger.log(`Tokens address: ${process.env.ASPO_ERC20_CONTRACT}`);
    ...
    for (let batch = process.env.ASPO_START_BATCH; batch < numBatch + 1; batch++) {
        start = 5000 * batch;
        end = start + 5000;
        if (batch == numBatch) {
            end = numBlock + 1;
            ...
            for (var i = 0; i < nft_numTransaction; i++) {
                for (var j = 0; j < token_numTransaction; j++) {
                    if (nft_ev_query[i].transactionHash == token_ev_query[j].transactionHash) {
                        ...
                        writeQuery = `INSERT INTO Aspo_aspo
                                      VALUES ('${transactionHash}', ${logIndex}, '${sender}', '${receiver}', ${value})`;
                        myLogger.log(`Add Transfer ERC20 with (transactionHash - logIndex): (${transactionHash} - ${logIndex})`)
